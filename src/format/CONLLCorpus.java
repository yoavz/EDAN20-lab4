        ...
        for (List<Word> sentence : sentenceList) {
            for (Word word : sentence) {
                if (word.getDeprel().equals("SS")) {
                    String subject = word.getForm();
                    int verbIndex = word.getHead();
                    String verb = sentence.get(verbIndex).getForm();
                    for (Word possibleObject : sentence) {
                        if (possibleObject.getDeprel().equals("OO") && possibleObject.getHead() == verbIndex) {
                            String object = possibleObject.getForm();
                            Triple t = new Triple(subject, verb, object);
                            tripleStore.add(t);
                        }
                    }
                } 
